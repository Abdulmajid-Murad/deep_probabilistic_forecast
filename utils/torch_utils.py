...
class ForecastDataset(Dataset):
    ...
    def __init__(self, X_train, y_train, sequence_length, linear_input, stride=1):
...
def torch_loader(X_train, y_train, X_test, y_test, sequence_length, batch_size,  linear_input=False):

    train_set = ForecastDataset(X_train, y_train, sequence_length, linear_input)
    train_loader = DataLoader(train_set, batch_size=batch_size, drop_last=False, shuffle=False, num_workers=0)

    test_set = ForecastDataset(X_test, y_test, sequence_length, linear_input)
